...
class DNC(nn.Module):
    ...
    def __init__(
        ...
        for layer in range(self.num_layers):
            if self.rnn_type.lower() == 'rnn':
                ...
                self.rnns.append(nn.Linear((self.nn_input_size if layer == 0 else self.nn_output_size),
    ...
    def _layer_forward(self, input, layer, hx=(None, None), pass_through_memory=True):
        ...
        if self.rnn_type == "fc":
            input = self.rnns[layer](input.unsqueeze(1))
        else:
    ...
    def forward(self, input, hx=(None, None, None), reset_experience=False, pass_through_memory=True):
            ...
            for layer in range(self.num_layers):
                ...
                def clone(x):
                    if type(x) == list:
                        ...
                        return x.clone()
                all_mems.append({k: clone(v) for k,v in mem_hidden.items()})
        ...
        if self.debug:
            return outputs, (controller_hidden, mem_hidden, read_vectors), all_mems, viz
        else:
            return outputs, (controller_hidden, mem_hidden, read_vectors), all_mems
